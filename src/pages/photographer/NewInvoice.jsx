...
export default function NewInvoice() {
    ...
    /*
     * Load all data required to create an invoice.
     *
     * Ownership is enforced by Supabase RLS. We do not rely on
     * client-side photographer_id filtering for security.
     */
    useEffect(() => {
        if (!user) {
            setLoading(false);
    ...
    async function loadInvoiceData() {
        try {
            ...
            /*
             * Get the photographer profile first.
             */
             ...
             * Load clients.
             *
             * RLS ensures the authenticated photographer only sees
             * their own clients.
             */
            ...
            /*
             * Load client profiles separately because profiles has
             * its own RLS policy.
             */
            const clientUserIds = [
                ...new Set(
            ...
            const formattedClients = (clientData || []).map((client) => ({
                ...client,
                profile: profileMap.get(client.user_id) || null,
            }));

            /*
             * Load bookings belonging to this photographer.
             *
            ...
            setError(
                err.message ||
                "Unable to load the information required to create an invoice."
            );
        } finally {
            setLoading(false);
        }
    ...
    }, [subtotal, formData.tax_rate, formData.tax_included]);
    ...
    }, [subtotal, taxAmount, formData.tax_included]);
    ...
    function removeItem(index) {
        if (items.length === 1) {
            setError("An invoice must contain at least one invoice item.");
            return;
        }
    ...
    function validateForm() {
        ...
        /*
         * booking_id is NOT NULL in the invoices database table,
         * therefore a booking is required when creating an invoice.
         */
        ...
        for (let index = 0; index < items.length; index += 1) {
            ...
            if (
                ...
                return `Quantity for invoice item ${index + 1} must be a positive whole number.`;
            }
            ...
            if (
                ...
                return `Unit price for invoice item ${index + 1} cannot be negative or empty.`;
            }
        }
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            /*
             * Verify that the selected client belongs to this
             * photographer's available client list.
             *
            ...
            /*
             * Verify that the selected booking belongs to
             * the selected client.
             */
            const selectedBooking = selectedClientBookings.find(
                (booking) =>
                    booking.booking_id === formData.booking_id
            );
            ...
            const { data: invoice, error: invoiceError } =
                await supabase
                    ...
                    .insert({
                        ...
                        tax_rate: Number(
                            Number(formData.tax_rate).toFixed(2)
                        ),
                        tax_included:
            ...
            /*
             * Create invoice items using the newly created invoice ID.
             */
            ...
            /*
             * If invoice items fail, remove the invoice that was
             * just created so an incomplete invoice is not left behind.
             */
            if (itemsError) {
                await supabase
                    .from("invoices")
        ...
        return (
            <main className="new-invoice-page">
                <div className="new-invoice-state">
                    ...
                    <h2>Loading invoice form...</h2>

                    <p>
                        Preparing your clients, bookings and
                        services.
                    </p>
                </div>
            </main>
            ...
            <header className="new-invoice-header">
                <div>
                    ...
                    <p className="new-invoice-description">
                        Create an invoice for a photography
                        client and their booking.
                    </p>
                </div>
                ...
                <section className="invoice-form-section">
                    <div className="invoice-section-heading">
                        ...
                        <div>
                            ...
                            <p>
                                Add photography services or
                                custom line items to this
                                invoice.
                            </p>
                        </div>
                    </div>
